...
type IssQuery struct {
	...
	Content     string `json:"content,omitempty"`                // 记录内容,可以选填部分json字段，查询符合这些字段的记录
	Notes       string `json:"notes,omitempty"`                  // 记录注释
	Rangee      string `json:"rangee,omitempty"`                 // 范围查询条件,作为查询条件，范围查询条件
	...
	/// Range范围查询条件(string)
	BHeight    string `json:"b_height,omitempty"`    // 区块高度,条件范围，区块高度范围
	CommitTime string `json:"commit_time,omitempty"` // 记录时间,条件范围，记录时间范围
...
type Info struct {
	...
	InfoVersion int    `json:"info_version"` // 信息版本号
	State       int    `json:"state"`        // 记录状态
	Content     string `json:"content"`      // 记录内容
	Notes       string `json:"notes"`        // 记录注释
	...
	BHeight     int    `json:"b_height"`     // 区块高度
...
type IssResponse struct {
	...
	MchSign  string `json:"mch_sign"  validate:"required"` // 通讯方签名
	// 业务参数
	Retcode    int    `json:"retcode"`     // 返回状态码，0表示成功，其它为失败
	Retmsg     string `json:"retmsg"`      // 返回信息，如非空，为错误原因。
	TotalInfos int    `json:"total_infos"` // 记录总数,符合条件的记录总条数
...
func QueryIss(iss IssQuery) (*IssResponse, error) {
	...
	json.Unmarshal(body, &issResponse)
